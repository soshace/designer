	...
	<td class="thumb">
		...
		<?php if ( $_product ) : ?>
			<?php if ($item["wcla_design_id"]):?>				
					<?php
					$design_data = wcla_load_design_data($item["wcla_design_id"]);					
			        ...
			        foreach ($design->data->locations as $location) {
			            ...
			            $output.='<embed style="width:50px;height:50px;max-width:587px;padding:0;margin:0;" src="' . DESIGNER_DESIGNS_URL . 'image_' . $item["wcla_design_id"] . '_' . $location->name . '_.svg" type="image/svg+xml" pluginspage="http://www.adobe.com/svg/viewer/install/"></embed><a  href="' . DESIGNER_DESIGNS_URL . 'image_' . $item["wcla_design_id"] . '_' . $location->name . '_.svg" target="_blank">Preview</a>';
			        }
			        echo $output;
			        ?>
			<?php else:?>	
